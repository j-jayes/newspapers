...
- Goldin & Katz (2008) argue that the demand for skilled labor drives educational attainment.

- Evidence is from wages > arguing that wages are a proxy for demand.
...
- But there isn't compelling evidence that the skills were demanded by employers directly.

- It could be that the education system expanded, and people gained new skills, which firms then adapted to use.

- We want to test this by looking at job adverts in Swedish newspapers.
...
- We want to measure the skills demanded by employers over time, after a new technology is introduced.
...
- A great paper by Tobias Karlsson in my department looking at the decline of messenger boys. They find that the introduction of new office technologies led to a decline in messenger jobs.
...
- They use a simple keyword search to count the relative frequency of selected job titles in the Swedish newspaper database. We think that we can do better, capturing skills demanded by employers in a more structured way.
...
- The jobtech API enriches job advertisements with structured data, identifying occupations, competencies, traits, and geographical locations from unstructured text.

- Does not work with historical job ads, but we think that we can adapt the method to do so.
...
With the Jobtech API, we can see that the demand for digital skills (.e.g. powerpoint, excel) in job ads increased significantly from 2006 to 2020, with the public sector lagging behind the private sector.
...
Newspapers in Sweden have been digitized and are available online, providing a rich source of historical job advertisements.

They have many job ads, but they require some work to extract structured data from them.
...
Close up, we can see the first word is often the job title, and there is a short description of the job.
...
Goal: Extract (just) Job Titles from the Swedish Newspaper Database
...
1.  We manually label 1,200 job titles in a training dataset.
...
XLM-R through huggingface. Fine tuned with 1,000 job ads, 200 in test set.
...
Text generation is nearly as accurate as NER, but it is so much easier to implement!
...
::: {.columns}
::: {.column}
![Melissa Dell's American Stories (2023)](assets/dell_layouts.png)
